...
import android.hardware.keymaster;

/**
 * This is an illustration of how you can use the Bouncy Castle ASN.1 parser to
 * extract information from an Android attestation data structure. On a secure
 * server that you trust, create similar logic to verify that a key pair has
 * been generated in an Android device. The app on the device must retrieve the
 * key's certificate chain using KeyStore.getCertificateChain(), then send the
 * contents to the trusted server.
 *
 * In this example, the certificate chain includes hard-coded excerpts of each
 * certificate.
 *
 ...
 *
 * 2. Verifying the certificate chain, up to the root. Note that this example
 * does NOT require the root certificate to appear within Google's list of root
 * certificates. However, if you're verifying the properties of hardware-backed
 * keys on a device that ships with hardware-level key attestation, Android 7.0
 * (API level 24) or higher, and Google Play services, your production code
 * should enforce this requirement.
 *
 * 3. Extracting the attestation extension data from the attestation
 * certificate.
 *
 * 4. Verifying (and printing) several important data elements from the
 * attestation extension.
 *
 * Although this sample code doesn't check the revocation status of intermediate
 * and root certificates, you should do so in production-level code. Note that
 * attestation certificates don't have certificate revocation lists.
 */
public class KeyAttestationExample {

    private static final String KEY_DESCRIPTION_OID =
    ...
    public static void main() throws Exception {
        ...
        // if attestation version is not the expected value,
        // then the extension data may be structured in a format that the parser
        // cannot recognize or accept.
        int attestationVersion = getIntegerFromAsn1(
                extensionData.getObjectAt(ATTESTATION_VERSION_INDEX));
        if (attestationVersion != EXPECTED_ATTESTATION_VERSION) {
            throw new Exception("Unexpected attestation version. "
                    + "Unable to parse extension data.");
        }

        // Use the attestation and keymaster security levels to determine
        // whether the device HAS A TEE
        // and whether the attestation certificate was generated in that TEE.
        // In this example, Device has TEE, but the certificate was generated in software.
        int attestationSecurityLevel = getIntegerFromAsn1(
                extensionData.getObjectAt(ATTESTATION_SECURITY_LEVEL_INDEX));
        int keymasterSecurityLevel = getIntegerFromAsn1(
                extensionData.getObjectAt(KEYMASTER_SECURITY_LEVEL_INDEX));
        System.out.println("Attestation security level: "
                + securityLevelToString(attestationSecurityLevel));
        System.out.println("Keymaster security level: "
                + securityLevelToString(keymasterSecurityLevel));
        ...
        // generated by the server and sent to the device.
        byte[] attestationChallenge = ((ASN1OctetString)
                extensionData.getObjectAt(ATTESTATION_CHALLENGE_INDEX))
                .getOctets();
        ...
        //detailed key description seperated on two authorization lists (TEE, Android key store) which describes info about key material, location and permissions
        ASN1Encodable[] softwareEnforced = ((ASN1Sequence)
                extensionData.getObjectAt(SW_ENFORCED_INDEX)).toArray();
        ASN1Encodable[] teeEnforced = ((ASN1Sequence)
                extensionData.getObjectAt(TEE_ENFORCED_INDEX)).toArray();

        // Since this key is TEE-based, we expect to find the key size in the
        // TEE list.
        int keySize = getIntegerFromAsn1(findAuthorizationListEntry(
                teeEnforced, KM_TAG_KEY_SIZE));
        ...
        int algorithm = getIntegerFromAsn1(findAuthorizationListEntry(
                teeEnforced, KM_TAG_ALGORITHM));
        System.out.println("Key cryptographic algorithm: " + algorithm);
        if (algorithm != KM_ALGORITHM_EC) {
            throw new Exception("This key is not an elliptic curve (EC) key, "
                    + "which was expected.");
        }

        //this key MUST be generated inside keystore, not imported
        //origin - describes it info
        int origin = getIntegerFromAsn1(findAuthorizationListEntry(
                teeEnforced, KM_TAG_ORIGIN));
        ...
        // them.
        ASN1Set purposes = (ASN1Set) findAuthorizationListEntry(
                teeEnforced, KM_TAG_PURPOSE);
        System.out.println("Key purpose(s): " + purposes);
        if (purposes == null
                || purposes.toArray().length != 1
                || getIntegerFromAsn1(purposes.toArray()[0])
                != KM_PURPOSE_SIGN) {
        ...
        int authTimeout = getIntegerFromAsn1(findAuthorizationListEntry(
                teeEnforced, KM_TAG_AUTH_TIMEOUT));
        System.out.println("Key authorization timeout: " + authTimeout);
        if (authTimeout != 300) {
            throw new Exception("This key does not have "
                    + "the expected authorization timeout.");
        }

        // The user can authenticate with either a password or a fingerprint
        // to authorize this key for use.
        int authTypes = getIntegerFromAsn1(findAuthorizationListEntry(
                teeEnforced, KM_TAG_USER_AUTH_TYPE));
        System.out.println("Key user authentication options: " + authTypes);
        if (authTypes != (HW_AUTH_FINGERPRINT | HW_AUTH_PASSWORD)) {
            throw new Exception("This key does not support "
                    + "the expected authentication options.");
        }

        // rollback-resistance key is garanteed to be gone permanently
        boolean rollbackResistant = (null != findAuthorizationListEntry(
                teeEnforced, KM_TAG_ROLLBACK_RESISTANT));
    ...
    extractAttestationSequence(X509Certificate attestationCert)
        ...
        byte[] attestationExtensionBytes = attestationCert
                .getExtensionValue(KEY_DESCRIPTION_OID);
        if (attestationExtensionBytes == null
                || attestationExtensionBytes.length == 0) {
            throw new Exception("Couldn't find the keystore attestation "
                    + "extension data.");
        }
    ...
    private static void verifyCertificateChain(X509Certificate[] certs)
        ...
        if (Arrays.equals(secureRoot.getTBSCertificate(),
                certs[certs.length - 1].getTBSCertificate())) {
            System.out.println("The root certificate is correct, so this "
            ...
            System.out.println("The root certificate is NOT correct. The "
    ...
    private static X509Certificate[] loadCertificates()
        ...
        X509Certificate[] certs =
                new X509Certificate[SAMPLE_ATTESTATION_CERT_CHAIN.length];
        CertificateFactory factory = CertificateFactory.getInstance("X.509");
        for (int i = 0; i < SAMPLE_ATTESTATION_CERT_CHAIN.length; ++i) {
            certs[i] = (X509Certificate) factory.generateCertificate(
                    new ByteArrayInputStream(
                            SAMPLE_ATTESTATION_CERT_CHAIN[i].getBytes()));
        }
        return certs;
    }

    private static String securityLevelToString(int securityLevel)
            throws Exception {
    ...
    public static int getIntegerFromAsn1(ASN1Encodable asn1Value)
        ...
        if (asn1Value instanceof ASN1Integer) {
            return KeyAttestationExample.bigIntegerToInt(((ASN1Integer)
                    asn1Value).getValue());
        } else if (asn1Value instanceof ASN1Enumerated) {
            return KeyAttestationExample.bigIntegerToInt(((ASN1Enumerated)
                    asn1Value).getValue());
        } else {
            throw new Exception("Integer value expected; found "
